...
function validateSchema(body, schema) {
  ...
  for (const [key, spec] of Object.entries(schema)) {
    ...
    if (body[key] !== undefined) {
      if (spec.type === 'string' && typeof body[key] !== 'string') return `${key} must be a string`;
      if (spec.type === 'number' && typeof body[key] !== 'number') return `${key} must be a number`;
      if (spec.type === 'boolean' && typeof body[key] !== 'boolean') return `${key} must be a boolean`;
      if (spec.type === 'string[]' && (!Array.isArray(body[key]) || !body[key].every(x => typeof x === 'string'))) {
...
function auditLog(entry) {
  ...
  fs.appendFile(AUDIT_LOG_PATH, record, (err) => {
    if (err) console.error(`[agentd] audit write error: ${err.message}`);
  });
  // Rotation check
  try {
  ...
  return new Promise((resolve, reject) => {
    const child = execFile(cmd, args, {
      ...
      if (err && !stdout) {
        ...
        resolve({ stdout: stdout.slice(0, MAX_OUTPUT), stderr: stderr.slice(0, 2000), code: err ? err.code : 0 });
      }
    });
  });
}
...
routes['POST /fs/read'] = async (body) => {
  ...
  return { path: body.path, lines: lines.length, content: lines.slice(offset, offset + limit).join('\n') };
};
...
routes['POST /git/commit'] = async (body) => {
  ...
  if (err) throw { status: 400, message: err };
  if (!isAllowedPath(body.repo)) throw { status: 403, message: 'repo not allowed' };
  if (body.message.length < 3) throw { status: 400, message: 'commit message too short' };
  if (body.message.length > 500) throw { status: 400, message: 'commit message too long (max 500)' };
...
routes['POST /docker/ps'] = async (body) => {
  ...
  const result = await execCommand('docker', ['ps', '--format', '{{.Names}}\t{{.Status}}\t{{.Image}}']);
...
routes['POST /docker/restart'] = async (body) => {
  ...
  if (err) throw { status: 400, message: err };
  if (!isAllowedContainer(body.container)) throw { status: 403, message: `container not in whitelist: ${body.container}` };
...
routes['POST /project/test'] = async (body) => {
  ...
  if (fs.existsSync(path.join(resolved, 'pytest.ini')) || fs.existsSync(path.join(resolved, 'pyproject.toml'))) {
    cmd = 'python3';
    ...
    throw { status: 400, message: 'no test runner detected (need pytest.ini/pyproject.toml or package.json)' };
  }
  ...
  try {
    ...
    if (output.length > MAX_OUTPUT) {
      ...
      return { job_id: jobId, summary: output.slice(0, 2000) + '\n... (full output in artifact)', artifact: logPath };
    }
...
function parseBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      data += chunk;
      if (data.length > 1024 * 1024) { // 1MB max body
