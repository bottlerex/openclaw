...
function getEvaluatedBindingsForChannelAccount(
  ...
  if (cache.byChannelAccount.size > MAX_EVALUATED_BINDINGS_CACHE_KEYS) {
    // Evict oldest 10% of keys (simple LRU approximation) instead of clearing all
    const toEvict = Math.floor(MAX_EVALUATED_BINDINGS_CACHE_KEYS * 0.1);
    const keys = cache.byChannelAccount.keys();
    for (let i = 0; i < toEvict; i++) {
      const oldestKey = keys.next().value;
      if (oldestKey != null) {
        cache.byChannelAccount.delete(oldestKey);
      }
    }
  }
...
export function resolveAgentRoute(input: ResolveAgentRouteInput): ResolvedAgentRoute {
  ...
  // Optimization 3: Pre-build Agent ID Map for O(1) lookup instead of O(n) scan
  const agents = listAgents(input.cfg);
  const agentIdMap = new Map<string, string>();
  for (const agent of agents) {
    const normalized = normalizeAgentId(agent.id);
    if (normalized) {
      agentIdMap.set(normalized, agent.id.trim());
    }
  }
  ...
  const choose = (agentId: string, matchedBy: ResolvedAgentRoute["matchedBy"]) => {
    // Use pre-built agentIdMap for O(1) lookup
    const trimmed = (agentId ?? "").trim();
    let resolvedAgentId: string;
    if (!trimmed) {
      resolvedAgentId = sanitizeAgentId(resolveDefaultAgentId(input.cfg));
    } else {
      const normalized = normalizeAgentId(trimmed);
      const foundId = agentIdMap.get(normalized);
      resolvedAgentId = sanitizeAgentId(foundId || (agents.length === 0 ? trimmed : resolveDefaultAgentId(input.cfg)));
    }
