...
// Optimization 4: Track last stat time to throttle fs.statSync calls in hot path
const STAT_THROTTLE_TRACKER = new Map<string, number>();
const STAT_THROTTLE_MS = 22_500; // TTL half (45s/2)
...
export function loadSessionStore(
  ...
  if (!opts.skipCache && isSessionStoreCacheEnabled()) {
    ...
    if (cached && isSessionStoreCacheValid(cached)) {
      // Optimization 4: Throttle statSync calls within TTL half-time
      const now = Date.now();
      const lastStatAt = STAT_THROTTLE_TRACKER.get(storePath) ?? 0;
      if (now - lastStatAt < STAT_THROTTLE_MS) {
        // Within throttle window; skip stat and assume cache is valid
        return structuredClone(cached.store);
      }
      // Time to re-stat; update tracker
      STAT_THROTTLE_TRACKER.set(storePath, now);
