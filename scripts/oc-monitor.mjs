...
function statusIcon(running) { return running ? `${GREEN}●${RESET}` : `${RED}○${RESET}`; }
function timeAgo(ms) {
  if (!ms) return "never";
...
function renderDashboard() {
  ...
  if (compact) {
    const chSummary = Object.entries(channels).map(([id, ch]) =>
      `${id}:${ch.running ? "UP" : "DOWN"}`
    ).join(" ");
    const sessCount = sessions.length;
    const nodeCount = nodes.length;
    process.stdout.write(`\r${DIM}${now}${RESET} conn:${GREEN}${connId.substring(0,8)}${RESET} ch:[${chSummary}] sess:${sessCount} nodes:${nodeCount} events:${eventCount} ${chatActive ? `${YELLOW}[chat]${RESET}` : ""} last:${lastEvent}    `);
    return;
  }
  ...
  lines.push(`${BOLD}╠══════════════════════════════════════════════════════════════╣${RESET}`);
  lines.push(`${BOLD}║${RESET} ${DIM}Time:${RESET} ${now}  ${DIM}Conn:${RESET} ${GREEN}${connId.substring(0, 12)}${RESET}  ${DIM}Events:${RESET} ${eventCount}  ${chatActive ? `${YELLOW}[CHAT ACTIVE]${RESET}` : `${DIM}idle${RESET}`}`);
  lines.push(`${BOLD}╠══════════════════════════════════════════════════════════════╣${RESET}`);
  ...
  for (const [id, ch] of Object.entries(channels)) {
    ...
    lines.push(`${BOLD}║${RESET}   ${icon} ${CYAN}${id}${RESET}${mode}${err}${DIM}${inbound}${RESET}`);
  }
  lines.push(`${BOLD}╠══════════════════════════════════════════════════════════════╣${RESET}`);
  ...
  if (sessions.length > 8) lines.push(`${BOLD}║${RESET}   ${DIM}... +${sessions.length - 8} more${RESET}`);
  lines.push(`${BOLD}╠══════════════════════════════════════════════════════════════╣${RESET}`);
  ...
  for (const n of nodes) {
    ...
    lines.push(`${BOLD}║${RESET}   ${GREEN}${name}${RESET} ${DIM}[${caps}]${RESET} ${DIM}${n.platform || ""}${RESET}`);
  }
  lines.push(`${BOLD}╠══════════════════════════════════════════════════════════════╣${RESET}`);
...
function connect() {
  ...
  ws.on("message", (data) => {
    ...
    if (msg.type === "res") {
      ...
      if (p) { pending.delete(msg.id); if (msg.ok) p.resolve(msg.payload ?? msg); else p.reject(new Error(p.method + " failed")); }
      return;
    }
  ...
  ws.on("error", (e) => {
    if (!compact) console.error("[ERR]", e.message);
  });
  ws.on("close", () => {
    if (!compact) console.error("[RECONNECT] in 3s...");
...
async function refresh(ws) {
  try {
    ...
    if (!compact) console.error("[ERR] refresh:", e.message);
  }
}
...
function handleEvent(msg, ws) {
  ...
  if (evt === "agent") {
    if (p.stream === "lifecycle" && p.data?.phase === "start") {
      ...
      if (eventsOnly) console.log(`[${new Date().toLocaleTimeString()}] ${evt} lifecycle:start runId=${currentRunId?.substring(0, 8)}`);
      renderDashboard();
    } else if (p.stream === "assistant") {
      const text = p.data?.delta || p.data?.text || "";
      lastEvent = `${CYAN}assistant:${RESET} ${text.substring(0, 60)}`;
      if (eventsOnly) process.stdout.write(text);
      renderDashboard();
  ...
  if (evt === "device.pair.requested") {
    ...
    if (eventsOnly) console.log(`[${new Date().toLocaleTimeString()}] ${evt} device=${p.deviceId?.substring(0, 12)} platform=${p.platform}`);
    renderDashboard();
    return;
  }
  ...
  if (eventsOnly) console.log(`[${new Date().toLocaleTimeString()}] ${evt}`);
  renderDashboard();
}
